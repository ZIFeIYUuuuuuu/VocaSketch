...
export function ProcessPlaybackPlayer({ process, finalSrc, elapsedMs, isLightMode }: ProcessPlaybackPlayerProps) {
  ...
  useEffect(() => {
    ...
    const size = Math.max(360, Math.floor(Math.min(rect.width || 720, rect.height || rect.width || 720)));
...
function drawProcess(
  ...
  const { actions, elapsedMs, image, isLightMode, size } = input;
  drawPaper(ctx, size, isLightMode);

  const cursor: CursorState = { x: size * 0.5, y: size * 0.5, radius: size * 0.018, tool: 'idle', visible: false };
  for (const action of actions) {
    ...
    if (action.type === 'stroke') {
      drawStrokeAction(ctx, action, size, progress, cursor);
    } else if (action.type === 'fillRegion') {
      drawFillAction(ctx, action, image, size, progress, cursor);
    } else if (action.type === 'maskReveal') {
      drawMaskRevealAction(ctx, action, image, size, progress, cursor);
    } else if (action.type === 'layerBadge') {
      drawLayerBadge(ctx, action, size, progress);
    } else if (action.type === 'finalReveal') {
      drawFinalReveal(ctx, action, image, size, progress, cursor);
    } else if (action.type === 'eyeSpark') {
      drawEyeSpark(ctx, action, image, size, progress, cursor);
    }
  }

  drawCursor(ctx, cursor, size, isLightMode);
}
...
function drawPaper(ctx: CanvasRenderingContext2D, size: number, isLightMode: boolean) {
...
function drawStrokeAction(
  ...
  const points = action.points.map((point) => ({ x: point.x * size, y: point.y * size }));
  ...
  ctx.lineWidth = Math.max(1.2, action.strokeWidth * size);
  ctx.beginPath();
  ...
  cursor.radius = Math.max(size * 0.013, action.strokeWidth * size * 2.2);
...
function drawFillAction(
  ...
  const centerX = action.center.x * size;
  const centerY = action.center.y * size;
  const radiusX = action.radius.x * size * eased;
  const radiusY = action.radius.y * size * eased;
  ...
  cursor.radius = Math.max(size * 0.035, Math.min(radiusX, radiusY) * 0.18);
  cursor.tool = action.tool;
  cursor.visible = progress < 1;
}
...
function drawMaskRevealAction(
  ...
  const radius = size * (0.18 + eased * 0.72);
  const centerX = action.phase === 'lighting' ? size * 0.55 : size * 0.47;
  const centerY = action.phase === 'lighting' ? size * 0.36 : size * 0.61;
  ...
  cursor.radius = size * (action.phase === 'lighting' ? 0.05 : 0.06);
  cursor.tool = action.tool;
  cursor.visible = progress < 1;
}
...
function drawLayerBadge(
  ...
  roundRect(ctx, size * 0.055, size * 0.84, size * 0.33, size * 0.062, size * 0.018);
  ctx.fill();
  ctx.fillStyle = '#e2e8f0';
  ctx.font = `${Math.max(11, size * 0.022)}px ui-monospace, SFMono-Regular, Menlo, monospace`;
  ctx.fillText(action.label, size * 0.077, size * 0.88);
  ctx.restore();
}
...
function drawFinalReveal(
  ...
  drawImageCover(ctx, image, size);
  ctx.restore();
  cursor.x = size * (0.45 + eased * 0.14);
  cursor.y = size * (0.55 - eased * 0.14);
  cursor.radius = size * 0.024;
  cursor.tool = action.tool;
  cursor.visible = progress < 1;
}
...
function drawEyeSpark(
  ...
  for (const point of action.points) {
    ...
    const glowRadius = size * (0.015 + 0.04 * eased);
  ...
  cursor.x = target.x * size;
  cursor.y = target.y * size;
  cursor.radius = size * 0.018;
  cursor.tool = action.tool;
  cursor.visible = progress < 1;
}
