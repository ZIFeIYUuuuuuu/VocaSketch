...
const LEGACY_API_BASE_URL = import.meta.env.VITE_LEGACY_API_BASE_URL ?? import.meta.env.VITE_API_BASE_URL;
...
async function apiRequest<T>(
  ...
  if (!ENABLE_LEGACY_V1_BACKEND || !LEGACY_API_BASE_URL) {
    throw new Error('Legacy v1 backend has been removed from the default VocaSketch flow. Use backend / api v2 instead.');
  }

  return performRequest<T>(toApiUrl(LEGACY_API_BASE_URL, path), options);
}
