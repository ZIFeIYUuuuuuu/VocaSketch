...
`backend/` 是 VocaSketch 当前默认的 Python-first v2 后端。旧 Node/V1 后端已经从正常运行流程移除，绘画任务、资产内容、SSE、runtime readiness、recent jobs 和 provider 边界都应从这里进入。
...
```bash
set VOCASKETCH_BACKEND_DATA_DIR=backend/data
set VOCASKETCH_BACKEND_HOST=127.0.0.1
set VOCASKETCH_BACKEND_PORT=8000
set VOCASKETCH_PROVIDER_PROFILE=mock
```

兼容说明：

- 推荐统一使用 `VOCASKETCH_BACKEND_DATA_DIR`、`VOCASKETCH_BACKEND_HOST`、`VOCASKETCH_BACKEND_PORT`
- 旧的 `VOCASKETCH_BACKEND_PY_*` 变量仍兼容，便于本地迁移
- 更早的 `VOCASKETCH_DATA_DIR` 目前也兼容，但不再推荐继续使用
